...
- **客服派单 = 会话执行人身份**：`jj-ralph`/`jj-same`/`jj-review` 的独占派单固定 spawn `agents/` 命名子代理（`jj-implementer`、`jj-researcher`、`jj-reviewer`），缺失时才回退 `general-purpose`；执行人只读独占派单文件，不读 parent 聊天或 skill `references/`。同 persona + 同 cwd 的后续切片用 `resume_from` 续接，不冷启动。
