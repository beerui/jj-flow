...
| WORKER_AGENT | team-worker | Prefer team-worker; fallback general-purpose (Codex) |
...
| Host mode | `team-session.json#host_mode` | Yes |
| Task list | `TaskList()` **or** `<session>/tasks.json` (degraded) | Yes (one of) |
| Active workers | session.active_workers[] | Yes |
| Role registry | session.roles[] | Yes |
| Message log | team_msg **or** `.msg/messages.jsonl` | Preferred |
...
**Worker Progress** (from message bus **or** file bus):

Before generating status output, read worker milestones:

- **full:** `team_msg` list progress/blocker (if maestro available)
- **degraded:** scan `<session>/.msg/messages.jsonl` for `type` in `progress` / `blocker` / `task_complete`; also read `tasks.json` statuses
...
```
Collect task states from TaskList() **or** tasks.json
  +- completedSubjects: status = completed
  +- inProgressSubjects: status = in_progress
  +- readySubjects: pending + all blockedBy resolved

Ready tasks found?
  +- NONE + work in progress -> report waiting -> STOP
  +- NONE + nothing in progress -> PIPELINE_COMPLETE -> handleComplete
  +- HAS ready tasks -> for each:
      +- Inner Loop role already active? -> SKIP spawn
      +- Mark in_progress (TaskUpdate **or** tasks.json)
      +- Log task_unblocked (team_msg **or** .msg/messages.jsonl)
      +- Spawn team-worker (or general-purpose fallback)
      +- On degraded hosts: prefer **one** spawn per wake (serial)
      +- Add to session.active_workers
      Update session + user [team] progress -> STOP
```

**Spawn worker** (prefer `team-worker`; else `general-purpose` with same prompt):
...
Agent({
  ...
  name: "<role>",
  run_in_background: true,  // if host lacks background, run serial and report
...
Use tasks.json + .msg file bus when Task*/maestro unavailable.
Do not write RALPH-*/DEL-* or advance ralph/dispatch gates.`
})
```
...
All tasks completed (no pending, no in_progress)
  ...
  +- Read session.completion_action:
      ...
      |   |   Output: "Resume with: Skill(skill='jj-team-coordinate', args='resume')"
      |   +- "Export Results":
