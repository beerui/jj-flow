# Command: session task chain (file: dispatch.md)

> **≠ `jj-dispatch`.** This creates tasks only inside a `TC-*` team session.  
> Does **not** create `DEL-*`, control-plane deliveries, or durable dispatch `task_key`.

## Purpose

Create **session** task chains from dynamic dependency graphs. Builds pipelines from the task-analysis.json produced by Phase 1. Workers are spawned as team-worker agents with role-spec paths.
...
- **Output**: Host Task* chain **or** `<session>/tasks.json` (degraded)
...
**A. full host** (TaskCreate / TaskUpdate available):

```
TaskCreate({ subject: "<PREFIX>-<NNN>", description: "..." })
TaskUpdate({ taskId, addBlockedBy: [...], owner: "<role-name>" })
```

**B. degraded host** (no Task*): write/merge the same subjects into `<session>/tasks.json`:
  ...
  "tasks": [
    {
      ...
      "status": "pending",
      "owner": "<role-name>",
      "blocked_by": ["<dep-subject>", "..."],
      "description": "PURPOSE: ... | Session: <session-folder> | RoleSpec: ...",
      "artifact": null
    }
  ]
}
...
See [references/host-codex.md](../../../references/host-codex.md).
