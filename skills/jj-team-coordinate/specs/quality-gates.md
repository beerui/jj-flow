# Session quality bands (team-local)

> **Not** ralph ACCEPT PASS and **not** a dispatch checkpoint.  
> Scores only gate worker retry / warnings inside a `TC-*` session.

## 1. Quality thresholds

| Band | Score | Action |
|--------|-------|--------|
| session_ok | >= 80% | Report completed (session quality only) |
| session_review | 60-79% | Report completed with warnings |
| session_retry | < 60% | Retry Phase 3 (max 2 retries) |
...
| 5 | Apply threshold -> session_ok / session_review / session_retry | Yes |

**On session_retry**: Retry Phase 3 (max 2 retries). After 2 retries, report `partial_completion`.

**On session_review**: Proceed with warnings logged to `<session>/wisdom/issues.md`.
