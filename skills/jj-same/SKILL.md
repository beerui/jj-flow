...
3. Write `assignments/ASSIGNMENT-RESEARCH-<target>.md` in **that target’s** Ralph (客服 shape: 来自 team-lead / 读这些 / 交付 / 不要改 / 先确认再开工; **exact paths**). Follow assignment-spawn.md, then Call `spawn_subagent` (`jj-researcher`; missing → `general-purpose`) **this turn per independent target** — `cwd` is that repo; exclusive input is that file; **调研 人设提示词 prefix** + file; read-only.
4. From each research report, write `assignments/ASSIGNMENT-HANDOFF-<target>.md` in that target Ralph. Before coding: 🔴 branch-purpose + CREATE freshness ([branch-purpose-preflight.md](references/branch-purpose-preflight.md)); `EXECUTION_READY` unmet → 🛑 **STOP**. LITE vs FULL is one decision ([LITE vs FULL](#lite-vs-full)). Follow assignment-spawn.md, then spawn `jj-implementer` (missing → `general-purpose`) **this turn per independent target** (different `project_id` / cwd; parallel) with the **实施 人设提示词 prefix** + that file only. Same-project writes stay serial. Do not wait for one target’s HANDOFF before spawning the others.
...
- Conversational: this-round `ASSIGNMENT-RESEARCH` / `ASSIGNMENT-HANDOFF` + matching **人设提示词 prefix**; spawn per assignment-spawn.md **this turn per independent target**; no parent port.
