...
**CREATE base freshness (EP-20260803)**: before creating a feature branch, `git fetch` the integration base and ensure the new branch tip is **not behind** `origin/<base>` (default `origin/master`). Allowed: **ff-only** update on a clean local base, or **branch directly from `origin/<base>`** (local master pointer need not move). **Forbidden**: silent `checkout -b` from a stale local tip when `behind_count > 0`; **forbidden**: `reset --hard` or unconfirmed rewrite of dirty/divergent local `master`. Do not modify unauthorized repos. Detail: [branch-purpose-preflight.md](branch-purpose-preflight.md) checks 6–10 / G6.
