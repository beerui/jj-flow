...
Missing latest source review / UAT PENDING / incomplete family plan is a delivery caveat by default, **not** an `EXECUTION_READY` block. User clearly says “start migration / implement / go” → `EXECUTE_NOW`: after fact check, conversational next action = ASSIGNMENT-HANDOFF spawn; mechanical/CLI = business code or focused tests.
