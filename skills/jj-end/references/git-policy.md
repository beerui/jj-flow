...
- **`.workflow/` is not dirty work.** It is the state root every engine of this product writes to — the ralph task loop, the dispatch manifests, the coordinate / lifecycle / swarm siblings — so it is dirty by definition while any of them is running. `preview` moves it out of the `unselected dirty paths` blocker and reports it as `workflow_state_paths[]` instead, and `execute`'s clean-tree check ignores it too. It stays listed on purpose: the reader sees what was set aside. **Do not relocate, commit or delete the engine state to make a closeout pass** — that used to be the workaround, and it broke the one record of in-flight work
