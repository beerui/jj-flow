...
## Policy (read on exception)

Integration priority, red-light blacklist, conflict classify, failure recovery, and dry-run field table → [git-policy.md](references/git-policy.md). Known or explicit `work_branch` must match the task’s actual branch purpose. Wrong root, detached HEAD, no remote, unfinished merge/rebase/cherry-pick/revert, dirty submodules or unselected dirty paths block batch execution. `.workflow/` is the exception and is reported as `workflow_state_paths[]` rather than blocking: it is this product's own state root, not project work — see the rule in [git-policy.md](references/git-policy.md) before relocating anything. Do not invent a branch purpose, integration history or product requirement.
