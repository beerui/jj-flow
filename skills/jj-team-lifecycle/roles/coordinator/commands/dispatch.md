...
RoleSpec: <skill_root>/roles/<role>/role.md
```

(`skill_root` = multi-host resolve for `jj-team-lifecycle`; never only `.claude/skills/team-lifecycle-v4`.)
...
- Owner: supervisor
- **NOT spawned as team-worker** — coordinator wakes the resident supervisor via SendMessage (degraded: serial supervisor turn)
- If `supervision: false` in team-session.json, skip creating CHECKPOINT tasks entirely
- RoleSpec in description: `<skill_root>/roles/supervisor/role.md`
