...
6. **Approved, branch/workspace confirmed (or unambiguous), and (Codex capabilities complete / or Grok Mode S/W)** → DISPATCH: persist intent(`PENDING_THREAD`) first → Codex: `CREATE_THREAD` → `BIND_THREAD`; Grok: **Mode S** bind current real session id (shared across task_keys allowed) + write attestation file; **Mode W** when isolation is required (exclusive-worktree, named branch tip, `execution_mode=W`). PREFLIGHT #5: Mode S + isolation → BLOCKED, plane unchanged.
...
| proposed_mode | Grok `S` (default, `project-branch`) or `W` (`exclusive-worktree`). Workspace column may still say `project-branch` / `exclusive-worktree`. |
