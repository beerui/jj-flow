...
import { buildExecutionDecision } from './executionDecision.mjs';
...
export function buildDispatch({ mode = 'auto', intent = '', evidence = [], cwd = process.cwd() } = {}) {
  ...
  const workflowPrompt = buildWorkflowPrompt({ recipe, intent, guardReport, evidence: normalizedEvidence, cwd });
  ...
  return {
    ...
    skill_calls: recipe.skillCalls,
    workflow_prompt: workflowPrompt
  };
}

export function buildWorkflowPrompt({ recipe, intent, guardReport, evidence, cwd }) {
  const calls = recipe.skillCalls
...
export function renderMarkdown(dispatch) {
  const calls = dispatch.skill_calls
