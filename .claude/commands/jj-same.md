...
- `grill-me` 只作为最后兜底。先检查需求、会话、Git、文档和源码；仍不明确时采用不扩大范围、不新增产品行为且可回退的最窄默认值并记录假设。只有缺失信息会改变 `MUST`、验收、目标集合或不可逆实现，且本地证据无法解除阻塞时才询问；有文档用 `grill-with-doc`，无文档才用 `grill-me`，仅有 Maestro 原生命令时用 `maestro-grill`。一次只问当前阻塞决策。
