...
- `skills/jj-same/` 定义同源迁移和持续同步协议。`SKILL.md` 是入口；`references/` 保存 handoff、项目族、产物路由和同步契约；`assignment.md` / `onboarding.md` 定义客服独占派单形状（`ASSIGNMENT-RESEARCH` → `ASSIGNMENT-HANDOFF`）与工人入职 spawn prefix；`scripts/` 负责采集源证据。
