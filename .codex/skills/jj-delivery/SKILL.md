...
3. 先给出证据清单和缺口，再给计划；证据不足的结论保持 `PENDING`。
4. 按需要调用 Maestro/Codex skills：分析、计划、执行、验证、审查、测试和知识沉淀。
5. 完成后报告修改内容、验证命令、未验证风险和需要用户确认的事项。
6. 实际修改代码且源项目验证通过后，将当前仓库交给 `$jj-same` 做 post-change discovery；仅当当前项目/分支存在 outgoing sync contract 或可同步同族目标时，展示候选并询问立即同步、延期或不适用。
