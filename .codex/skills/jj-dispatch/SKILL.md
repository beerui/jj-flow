...
标准任务（默认 `task_mode=standard`）在创建任何 Codex thread 前必须生成 `.workflow/tasks/<TASK-ID>/任务.md`、`plan.md`、`progress.md` 和 `result.md`；只有用户明确说 quick 才可以跳过完整任务目录。可用 `jj task scaffold --manifest ... --delivery ...` 生成任务目录。控制项目的 handoff、dispatch、reports、receipts 也必须按任务 ID 分目录，不能把大量 JSON/Markdown 平铺在根目录。
