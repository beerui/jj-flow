...
每个标准任务必须拥有一个稳定的 `task_id`，默认值为 `TASK-<delivery_id>`，也可以在 delivery 上显式指定 `TASK-...`。`jj task scaffold` 会在 `.workflow/tasks/<TASK-ID>/task.json` 写入任务索引，记录 `delivery_id`、控制面 manifest 相对路径和创建时间。

新会话收到一个 `TASK-ID` 时，先读取该索引和 `task.md`，再按索引中的 manifest 读取 delivery 的实时状态；不得要求用户重复提供需求正文、项目集合或历史状态。CLI 可直接执行 `jj task context --task TASK-ID` 获取可复制的最小上下文，或执行 `jj task status --task TASK-ID --json` 获取结构化状态。任务文档是上下文引用，控制面 manifest 才是状态真相源。

控制项目首次接收需求时必须先完成 intake，不得直接把模糊需求变成 PREVIEW。至少确认：需求归属项目（`requirement_owner`）、需求来源（`origin_project`）、本轮领头项目（`lead_project`）、明确目标集合（`targets`）、是否允许多目标，以及是否为 `quick` 任务。缺任一项时只返回 `INTAKE_REQUIRED`，等待用户选择。

标准任务（默认 `task_mode=standard`）在创建任何 Codex thread 前必须生成 `.workflow/tasks/<TASK-ID>/task.json`、`task.md`、`plan.md`、`progress.md` 和 `result.md`；只有用户明确说 quick 才可以跳过完整任务目录。可用 `jj task scaffold --manifest ... --delivery ...` 生成任务目录。控制项目的 handoff、dispatch、reports、receipts 也必须按任务 ID 分目录，不能把大量 JSON/Markdown 平铺在根目录。
...
命令只输出任务主标题、任务 ID、`PREVIEW → APPROVE → DISPATCH → TICK` 流程和下一步命令。任务文档正文、分发提示词和审计字段只传给对应 Agent，不作为用户完成反馈展示。
...
每条需要分发的责任都自动携带 `distribution_prompt`。它至少包含 `summary`、`source_project`、`source_head`、`handoff_ref`、`target_project`、`target_decision`、`acceptance_criteria`、`risk_points`、`do_not_port` 和 `unresolved`。Host action 会把同一结构化对象和 `initial_prompt` 传给子任务；子任务不得重新向用户询问已经在 control plane 中确认的源需求、目标或风险。
...
调度智能体可以请求 Codex App 创建角色会话，但子会话不能自行创建可见任务、修改控制 manifest 或扩大目标集合。开始前检测 `list_projects`、`list_threads`、`create_thread`、`read_thread`、`send_message_to_thread`、项目 worktree 和 sandbox capability；任一必需能力缺失时停在 `PREVIEW_ONLY/BLOCKED`。同一项目同时最多一个 active write task。全部流程。
...
源项目完成并通过验证后，默认只生成推荐下一步，不自动扩大目标集合。推荐必须逐项目列出 `DIRECT / ADAPT / BLOCKED`、source/target HEAD、风险、confidence、`handoff_ref` 和可携带的 `distribution_prompt`。用户选择后重新执行 PREVIEW 和 APPROVE，再创建目标分析任务；不能复用旧 approval 或静默创建目标 thread。
