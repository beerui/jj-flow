...
- 基本盘：`/path/to/org-a/project-a`
- 项目B：`/path/to/org-b/project-b`
- 项目C差异项目示例：`/path/to/org-a/project-e`
- 项目A功能分支：`feat/pa-silence-0710`
- 项目B功能分支：`feat/pb-silence-0710`
...
这个入口不能仅因为项目A没有就漏掉，也不能反向强加到没有二维码登录的项目。

## 关键差异与教训

### 1. 不要整块对齐 legacy

项目B会话曾为了“方便维护”把项目A旧控制流一并搬入，随后用户明确要求撤销：只让新修改逻辑一致，不动老逻辑。

迁移账本应记录：

- `MUST`：新增业务码、配置查询、loading、最终产品行为。
- `TARGET-ONLY`：项目B二维码入口、项目B既有 `validUser` 条件。
- `DO-NOT-PORT`：被撤销的 legacy 对齐。

### 2. 目标代码不必逐行相同

项目A与项目B最终都实现验证码优先和保留密码入口，但状态管理仍有差别。只要需求不变量、失败策略和验收一致，不应为了代码形状统一再改目标旧逻辑。
...
按用户给出的项目族角色，`project-e` 是本案例的项目C。它不是 Vue 2 ERP 基本盘的同构复制：
...
因此沉默账户需求对该项目不能判定为 `DIRECT`。接口中的 `/api/admin` 前缀不能改变其业务角色，但会改变迁移方式。在后端未说明 `/api/admin/login` 会返回 `1027`、且产品未要求项目C提供验证码登录前，应判定：

- 当前业务场景：`N/A`；或
- 若产品确认项目C也必须支持：`BLOCKED`，先补齐接口契约和验证码登录产品流程，再按 Vue 3 原生结构 `ADAPT`。

禁止把 Vue 2 的 `login.vue`、mixins 或 Element UI 弹框复制进项目C。

本案例只覆盖三个前台项目。项目A后管、项目B后管和项目C后管应在后管需求中单独建立调用链和迁移矩阵，不能从本案例推断它们也需要沉默账户逻辑。
