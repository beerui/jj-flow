...
- 领头分支不符合可判定格式时，先从家族计划、会话和 Git 历史验证。能够按“不扩大范围、只替换角色前缀”得到唯一可回退结果时记录假设并继续；只有仍存在多个会改变目标分支的有效解释时，才把 `grill-with-doc`、`grill-me` 或当前环境可用的 `maestro-grill` 作为最后兜底，一次只确认分支映射。
