...
6. **落盘**（复制 [review-report.skeleton.json](references/review-report.skeleton.json)）  
   ...
   - 维护场景优先：`jj ralph review-record … --source … [--host-review-json …]` 或 `ralph_ops.mjs review-record`（与直接写文件同 schema，**不丢溯源**）。
...
6. 跨项目正式闭环用调度入口（如 `$jj-dispatch` / `/jj-dispatch`），本 skill 不替代 VERIFIED 门。  
7. 规程与示例**不绑定**某一宿主产品名；入口发现用通用能力名。  
