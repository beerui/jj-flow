...
**出什么事：** 一有冲突就停；或者只解了 import 冲突，业务函数冲突还留着，留下半成品 merge。

**怎么做：**

1. 先列出每个冲突文件和建议处理方式；拿不准的交给你判断
2. 能一句话说明、无需替产品做决定的冲突，全部处理完再继续 push
3. 有真正判不了的冲突（例如同一开关两边相反、二进制或密钥）就整段中止，回到工作分支，把完整清单交给你
4. 解完不能留下 `<<<<<<<`
