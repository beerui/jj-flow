...
| 可重放反馈 | 4 | `jj scenario` 已覆盖 happy path、中断恢复、部分目标失败和 handoff 契约，统一 report 可做纯状态 replay |
| 可观测性 | 3 | `jj doctor --json` 提供仓库诊断，trace 保存每步输入、输出、revision、语义 hash 和 evidence refs；完整跨 host telemetry 仍延期 |
| 自主闭环 | 2 | Reviewer/Developer 返工闭环已存在；真实 host 试跑和自动恢复仍属于 M7 |
| 熵控制 | 0 | 尚无 doc-gardening、重复规则或陈旧知识扫描循环 |
...
- `docs/exec-plans/`：复杂实施计划的目标位置，分 `active/` 与 `completed/`；在真正执行第一项大型 Harness 改造时创建。
