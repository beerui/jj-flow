...
Claude Code 中使用对应 slash commands：
...
3. 在 `.codex/skills/` 和 `.claude/commands/` 增加或更新原生命令资产。
4. 再在 `src/recipes.mjs` 增加 recipe。
5. 在 `src/dispatch.mjs` 确认路由关键词。
6. 在 `src/guards.mjs` 增加必要 guard。
7. 在 `tests/` 增加路由、安装资产和 guard 测试。
