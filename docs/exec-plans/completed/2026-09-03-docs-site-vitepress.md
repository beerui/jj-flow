...
对话入口表（Codex `$jj-ralph` / 其他 `/jj-ralph`）
...
怎么算做完：验收通过并归档——目录搬进 `.workflow/ralph/completed/`，Agent 给一段短报告。归档后会询问是否把可复用结论写入全局知识库（须经你确认）。
...
## 进阶
强度档表（tiny / standard / strict）；写整齐的参数格式；卡住时的示例说法（换策略 / 先暂停 / 验收不算退回改）
...
- [x] **B3.1 init**：一句话（把当前仓写进全局地图 `~/.jj-flow/map.md`，需要时补知识库；不是开需求）；开始前（无）；第一次这样用（`$jj-init` → 短提案（中文名 / 别名 / 家族 / 待写入条数）→ 经你确认 → 写入 → 短报告）；常用说法（四句）；名字来源规则一句。
...
- [x] **B3.4 `docs/commands.md`**：总览表不动；"常用说法"合并为一组；末尾加「第一次用先看：第一次使用 → ralph」。
- [x] **B3.5 `docs/index.md`**：三步开始改为 安装 → 第一次使用 → 在业务仓对话里说一句话；删除与安装页重复的「对话入口」表（留一句 + 链接）；其余保留（含"项目族编排"定位语）。
- [x] **B3.6 `docs/installation.md`**：开头加一句"装的是对话入口，不是后台服务"（已有则保留）；把「装好后如何调用」表上移到「只装某一个工具」之前；下一步链接文字改「第一次使用」。
