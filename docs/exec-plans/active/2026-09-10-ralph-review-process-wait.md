...
**C-reviewer-exclusive-input-v1**（一次只做当前轮）
...
- [x] 落地后请加 `evals/regression/` 新项：`EP-20260910-review-assignment-exclusive.json`。
...
**C-ralph-round-budget-v1** — 对齐客服：同一文件夹活一辈子，复杂工作拆成当前当前轮，没有终身循环上限。

- 对话路径 `gate_set=full`：`deliver-attempt` **不**因 `max_iterations` / `max_deliver_loops` BLOCK。
- 当前当前轮 = 下一个未勾 Step；不要在同一轮开始后面的 Step。
...
- F-5：`scope --replace-in` 抬起上一轮 `STAGNATION`；补 PAUSED / ABANDONED resume 重置测试。
