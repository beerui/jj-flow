...
安装完成后，Codex 能识别 `$jj` skill。用户在任意项目对话里输入 `$jj delivery ...` 时，Codex 会加载 `skills/jj/SKILL.md`，再由该 skill 生成 `jj-flow` 调度结果。
...
1. Codex 识别 `$jj` skill。
2. 输出内容围绕 `delivery` 交付协议，而不是泛泛解释命令。
3. 当证据不足时，guard 保持 `PENDING`。
...
`$jj` 会优先整理项目上下文、资料来源、关键决策和 Maestro 调用链；只有边界、方案或权限真正阻塞时才追问。
...
CLI 是 `$jj` skill 内部生成结构化调度结果的实现方式，也是维护者测试 recipe、guard 和文档站的工具。普通交付用法以 Codex 内 `$jj` 为准。
