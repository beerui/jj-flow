...
CI（ubuntu）在 verify 前用 `.github/actions/prepare-lab-roots` clone `beerui/jj-lab-loop` / `beerui/jj-lab-family` 到 `$RUNNER_TEMP` 绝对路径，seed，再注入 `JJ_LAB_*_ROOT` + `JJ_FLOW_ROOT`。本地跑 `verify` 同样必须先设绝对根（或已存在的 `lab-roots.json`）。不要从产品 toplevel 猜 `../jj-lab-*`。
