...
CI 不跟 gym 的 `main`，而是钉在 `.github/actions/prepare-lab-roots/action.yml` 的 `loop_ref` / `family_ref` 两个 SHA 上。两个 gym 的机械 oracle 直接读写 ralph 产物（`tasks/<task-*>/task_plan.md`、`.state/run.json` 等），所以 **ralph 布局、`run_id` 形态或 gate 契约一变，就得同步升 pin**，否则 CI 只会在 `verify` 的最后一步 `lab:check` 报错，而且报的是 gym 侧的症状（例如 `acceptance.md` ENOENT、`legacy RALPH run layout for RALPH-*`），不是产品缺陷。

升 pin 的做法：

1. 先把 gym 的修复合进 gym 仓 `main`（gym 是独立仓，产品仓的 token 未必能推）。
2. 按上面的 clone 方式把两个 gym 都 checkout 到目标 SHA，seed 后在产品仓跑 `npm run lab:check`，确认 PASS。
3. 改 `action.yml` 两个 `default:`，再跑一遍 `npm run verify`。
4. 在 CHANGELOG 记一句「gym pin → `<sha>` / `<sha>`」，方便回溯。

注意区分两个不同的东西：`action.yml` 里的 pin 是 **gym 仓的 commit**；gym 自己的 `lab-manifest.json` 里的 `jj_flow_commit` 是 gym **声明兼容的产品 commit**，前者过旧才是 CI 红的原因。
