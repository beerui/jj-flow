...
- `$jj-dispatch` / `/jj-dispatch`：多项目调度入口。项目族顶层为 `D:/a`（其下为全部受控业务项目）；在业务仓会话发起；协调状态写入 `D:/a/dispatch-control`（可配置，按 `delivery_id` 分目录）。不要求打开或每波新建控制仓。Codex / Grok / Qoder 可装；**无** Claude 薄命令。
