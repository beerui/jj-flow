...
`grill-me` 只作为最后兜底，不是常规分析步骤。遇到信息缺口时，先从当前需求、会话、Git、项目文档和源码验证；仍不明确时，采用不扩大项目范围、不新增产品行为且可回退的最窄默认值，并在计划中记录假设。只有缺失信息会改变 `MUST`、验收标准、目标项目集合或不可逆实现，而且穷尽本地证据后仍无法继续时，才使用 `grill-with-doc`；没有可用文档时才使用 `grill-me`，环境仅提供 Maestro 原生命令时使用 `maestro-grill`。一次只询问当前阻塞决策，不展开非必要问题清单。
