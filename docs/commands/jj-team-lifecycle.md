...
**可选入口：** 只帮你安排这一轮怎么做，单独跑完不算验收通过；验收仍看 [ralph](jj-ralph.md) / [dispatch](jj-dispatch.md) 的记录。用固定角色和固定阶段把规格、计划、实现、测试、审查串起来，适合希望过程标准化的团队。
...
**别用在：** 想按问题动态拼角色用 [coordinate](jj-team-coordinate.md)；想比较多个候选方案用 [swarm](jj-team-swarm.md)；只改一个小需求直接用 [ralph](jj-ralph.md)。
