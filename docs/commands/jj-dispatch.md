...
**项目族顶层：** `D:/a`（其下是全部受控业务项目与 `map.md` / `config` / `knowledge`）。

**从哪发起：** 用户在 `D:/a` 下某个 **业务仓**（承接 / 兑接 / 承载…）会话里直接 `$jj-dispatch` / `/jj-dispatch`。**不要求**先打开「控制项目」。

**状态写哪：** 协调状态写入 `D:/a` 下 **共用** 的状态目录（默认 `D:/a/dispatch-control`，可配置）。多波次 = 多个 `delivery_id` 子目录。用户 cwd 保持业务仓；Agent 写状态根。
...
第一次理解多项目调度时，先点一遍：[多项目调度演示](dispatch-demo.html)（用承接 / 兑接 / 承载举例）。
...
```bash
jj task scaffold --manifest .workflow/dispatch/DEL-xxx/control-plane.json --delivery DEL-xxx --root .
```

生成位置为 `.workflow/tasks/TASK-DEL-xxx/task.md`、`plan.md`、`progress.md` 和 `result.md`。只有明确的 `quick` 任务可以跳过。handoff、dispatch、报告和 receipts 同样按任务 ID 分目录，避免平铺大量 JSON/Markdown。

分配前只向用户展示任务主标题和流程，不展示任务正文：

```text
jj task assign --manifest .workflow/dispatch/DEL-xxx/control-plane.json \
  --delivery DEL-xxx --task TASK-DEL-xxx
```

每个 `CREATE_THREAD` action 都携带结构化 `distribution_prompt` 和 `initial_prompt`，包括 source/target、source HEAD、handoff、目标决策、验收标准、风险、排除项和未解决项；子任务直接消费这些内容。

源项目完成后，系统先返回逐项目推荐下一步。推荐包含 `DIRECT / ADAPT / BLOCKED`、风险、confidence、source/target HEAD 和可复用参数。用户选择后必须重新 PREVIEW / APPROVE，才会创建目标分析和开发任务。
