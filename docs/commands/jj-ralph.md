...
小改动可以只走三步：**说清 → 改 → 收**。内部还是同一份任务账本、同一个目录，只是把「分析 + 计划」合成一步、「验收 + 归档」合成一步；验收证据的要求**一点不减**。
...
lite 和强度档是两回事：tiny 说的是「分析计划写多短」，lite 说的是「走几道关」；说 tiny **不会**自动变 lite。

判错了不用重来：lite 里任何一关没过、或者范围变大了（多改了别的文件 / 模块），系统自动升回 full，**同一条任务接着走**，已经写的东西都留着。lite 改代码的循环最多 3 轮，到顶就升 full。
