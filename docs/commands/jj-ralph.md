...
已知当前任务时，Agent 会直接读取该任务的阶段摘要，一次取得目标、计划、验证记录和改动范围。只有需要查具体规则时才读相应参考章节。定位和状态默认使用简明输出，详细信息仍可展开；交付仍保留五个门禁，deliver 和 finalize 复用已有批量操作。
...
每一步通过就自动进下一步，不会反复问你“要不要继续”。只有几种情况会停下来等你：分析时或 MUST / 范围 / 验收事后仍确认不了（先问，不要猜着做）、你说了“先不写代码”、要做不可逆的事（如推送）。
