...
```text
$jj-ralph [Image] 这里要改一下：放到列表对应列的下面，标题去掉
```
...
小改也走这五步；说 tiny 只让分析和计划写短，**不**少关。截图 / 「这里」先当需求看。说「先不写代码」时只写分析、先不动代码。
...
| 同会话接着做 | 「继续」「修完」 | 接着当前任务未完成的那一步，不新开 |
| 按审查改 | 「按审查改」 | 对着最新审查意见改，不从头分析；**不**新开「审查修复」任务 |
| 改坏了 | 「改坏了」 | 同一任务换做法，不新开 |
| 先只分析 | 「先不写代码」「先理解需求」 | 只写分析；你说「开始做吧」再改代码 |
| 截图指一下 | 「这里」+ 图 | 图里的界面就是需求 |
| 先不做了 | 「这个先不做了，砍了」「撤回修改」 | 标成废弃；以后再说「还要做」可救回 |
...
.workflow/ralph/
  index.md
  business-map.json
  task-…/           # 活跃：task_plan（Goal/验收/Steps）+ 按日 progress + findings；机器事件在 .state/events.jsonl
...
| strict / 要审查再归档 | 加严 |
| 截图 / 「这里」 | 图里的界面就是需求 |
| 先不写代码 / 先分析 | 只写分析，先不动代码 |
| 继续 / 按审查改 / 改坏了 | **同一任务**接着做（不新开） |
