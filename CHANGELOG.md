...
- **jj-end / jj-review 无异常只回一行**：收工落地或审查 `PASS` 时聊天不列表格、不堆字段；冲突分类表 / dry_run 表 / `BLOCKED` / `NEEDS_CHANGES` findings 仍保留。合约：`tests/jj-end-contract.test.mjs`、`tests/jj-review-contract.test.mjs`。
