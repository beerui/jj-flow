...
## [0.2.0](https://github.com/beerui/jj-flow/compare/jj-flow-v0.1.0...jj-flow-v0.2.0) (2026-09-07)
...
- **Ralph lean 验收仍挡弱证据**：`## 验收` checkbox（`[x]`=PASS）也走 `inspectAcceptanceEvidence`；`write-then-read` / `cross-path` / `runtime-env` 仅 `static`/`diff`/`rg` 或无证据仍挡 `accept PASS`。合约：`tests/jj-ralph-contract.test.mjs`。
- **Ralph 审查切片不是新任务**：现场 `task-h5-enter-review-fix` 与 `task-enter-form-h5` 是同一需求。`init` 拒绝 `审查修复` / `按审查` / `review-fix` 新 slug；`index.md` 写 `## 同需求提示`（同会话或审查切片并排），只提示不自动合并。Dispatch 对同会话或唯一审查切片走 `reuse-sibling`。合约：`tests/jj-ralph-contract.test.mjs`、`tests/task-artifacts.test.mjs`。
...
- **Dispatch 统筹 / 项目 Ralph 分轨**：`~/.jj-flow` 只写 plane / 回执 / `TASK-*` 索引（Goal / 验收 / Steps）；`jj task scaffold` / DISPATCH 必须在每个 lead/target 业务仓 `init` 或 `resume` 完整 Ralph `task-<slug>`。禁止再用 control 目录的 `ANL-LEAD` / `ANL-TARGET` / 「分发提示词」当实施本。合约：`tests/task-artifacts.test.mjs`、`tests/jj-dispatch-contract.test.mjs`。
- **Ralph `next` + index 归档提示**：`jj ralph status` / locate 带 `next`（`NEEDS_CHANGES`→`review`，accept PASS 且 latest 为 `working_tree` PASS→`commit-scoped-review`，可落地→`finalize`；resume 后有 `run.archive` 不误报必须立刻 finalize）。`index.md` 在活跃 `task-*` 超过 5 条或任一条 5 天未更新时写 `## 归档提示`，只提示不自动归档；PAUSED/BLOCKED/做到一半先问用户。`saveRun` 刷新 index（写失败不反噬 run.json）。合约：`tests/jj-ralph-contract.test.mjs`。
...
- **Ralph migrate 整理存量 `.migrated-*`**：根上 `.migrated-RALPH-*` 收进 `migrated/`；`tasks/` 遗留上提到根；COMPLETED/ABANDONED 再搬进 `completed/`。幂等，不自动删残骸。合约：`tests/jj-ralph-contract.test.mjs`。
- **Ralph 人读产物收成短合同**：`task_plan.md` 只留 Goal / 验收 / Steps（当前合约）；`progress.md` 按日追加人读叙事；机器事件只写 `.state/events.jsonl`。`findings.md` 改动摘要 + 行为 + 真踩坑 + 验证，禁止空 F-00N。`map-find` 不再把 title/goal 切成 keywords 灌进 `business-map.json`。Skill 约定：并行读、offset/limit、禁止重读已注入内容、禁止整读 business-map；Grok 默认不调 MasterGo MCP。Loop gym L1-S7a 改为同一合同（重写 live Goal，历史进 progress；不再要求 已落地/Landed）。`jj-review` 对照 `## Steps`（leftover `## 计划 → ### 当前`）；`review-record` 机器行进 events.jsonl，compliance 关闭条件不再要求往 live plan 堆已落地。合约：`tests/jj-ralph-contract.test.mjs`、`tests/memory-hot-layer.test.mjs`。
- **jj-ralph 对话路径弃用 lite**：真实口语几乎不用 lite / 顺手修。`$jj-ralph` 一律五 gate，不传 `--lite` / `brief` / `close`，忽略 init `gate_set?`。只留 intensity `tiny|standard|strict`（tiny 只缩短计划）。补真实说法：截图 / 「这里」先当需求；「先不写代码」停在 ANALYZE；同会话「继续 / 按审查改 / 改坏了」resume 禁止 init。机械 CLI `--lite` 仍兼容旧 run，本轮不拆 `src/ralph`。合约：`tests/jj-ralph-contract.test.mjs`。
- **jj-end 冲突默认自己合**：分类从 `simple`/`complex` 改为 `self-merge`/`unhandleable`。两边都在演进、能一句话说清怎么合的（Vue/文档/条件守卫不同，如 feat/dynamic-form 的 AGENTS.md、`readonly` vs `uploadDisabled`、草稿 helper、LOGO 条件）必须自己合完继续收工；只有同一产品开关两边相反且无法判断、二进制/密钥、读完仍无法陈述解法才整段 abort。禁止「看起来像逻辑就停」和子集解再 abort。G-end-2 改写。合约：`tests/jj-end-contract.test.mjs`。
- **Ralph 自动结案补录（§4.5 / §4.9.4–5）**：仓内补录 [ralph-auto-closeout](https://github.com/beerui/jj-flow/blob/main/docs/design-docs/ralph-auto-closeout.md)（标明非修订 10 原文；表格外 code span 用 `|`）。`locate` 行带 `next` / `warning` / `closeout`。`jj install-skill --platform agents`（`--platform all` 含此端）写入 `~/.agents/skills` + `commands`，安装时清 retired。`jj ralph remediate` 默认 dry-run，`--yes` 只处理 `finalize` 与 `migrate`。主路径仍见 PR #30。
- **Ralph 自动结案（auto-closeout）**：accept PASS 后 MUST `finalize`。`status` 带 `next`；`phase=ARCHIVE` 仍在活跃根打「未完成收尾」。`writeRalphIndex` 写失败降级。`jj ralph locate` 覆盖扁平 `task-*` 与 `completed/`。合入：PR #30 / `d377acb`。
...
- **Ralph 自动收尾（accept → finalize 同回合闭环）（规划草案，Proposed）**：`docs/design-docs/ralph-auto-closeout.md`。finalize 同回合 MUST 化 + gate 输出 `next` 提示（含 `ralph_ops.mjs` 输出层）+ `jj ralph status` 第二道告警；解析链按当前宿主排序（本宿主 → 仓内安装 → 其它宿主 → 全局 `jj` → npx）；`.state` 手写禁令；`gate_set=undefined` 修复；index 刷新下沉 `saveRun`（run.json 先落盘、index 写失败降级不反噬主写入；migrate 批路径豁免；jj-ralph Locate 与 jj-review 找任务都先读 index 活跃表，本修复是两者的共同依赖）；init 模板删 `> Status:` 叙事头，状态只活在 run.json + index + progress 当日轮次。本轮不改 `src/ralph/`。
...
- **Ralph 工作区 P2+a lite 档**：`jj ralph init --lite|--full`（`ralph_ops` 同）显式覆写 `gate_set`，默认仍 full，与 intensity 正交（tiny ≠ lite）；lite `budget.max_deliver_loops ≤ 3`，stagnation 照跑。`setGate` 接受别名 `brief`（analyze+plan）/ `close`（accept+archive），内部仍写五键；`close` 照走 accept/archive 证据门与 judgment 层，弱证据仍挡，可接 finalize。任一 lite gate FAIL/BLOCKED（`setGate` / `rollbackPhase`）或 `jj ralph scope --in` 新增 `scope.in` → 自动升 full（恢复 intensity 预算，不小于已用次数），同目录不换 `run_id`，progress 追 `promoted lite→full`。schema 不升 1.3。合约：`tests/jj-ralph-contract.test.mjs`。
- **Ralph 工作区 P2+b 启发式判档 + 文案**：`jj ralph init` 无 `--lite/--full` 时按规模**只建议**（`suggestGateSet`：`scope.in` ≤ 2 个具体文件或小改口语 ∧ 无架构词 ∧ 单一验收项才建议 lite，拿不准即 full；不读 intensity，tiny ≠ lite）；建议挂在返回对象 `run.gate_set_suggestion`（`applied=false`）+ progress 一行，文本模式打 `gate_set? lite …`，`ralph_ops init` 透出；**`run.json.gate_set` 无显式 `--lite` 永不变 lite**。skill / `docs/commands/jj-ralph.md` / `claude-commands`：「小改 / 顺手修」→ `--lite`，「完整走一遍」→ `--full`，没说 = full；`phases.md` 新增「Gate set (full / lite)」。合约：`tests/jj-ralph-contract.test.mjs`（P2+b 两例 + 文案标记）。
- **Ralph 工作区 P2+ lite exec-plan 关闭**：`docs/exec-plans/completed/2026-09-03-ralph-plans-workspace-p2-lite.md`（P2+a + P2+b 勾完并附验收证据）；设计 [Ralph 任务工作区 `.plans` 化改造](https://github.com/beerui/jj-flow/blob/main/docs/design-docs/ralph-plans-workspace.md) §3.8 / §4 P2+ 标落地，整体标 Implemented。
- **Ralph lite 预算出口解除过期 BLOCKED**：lite 在 `max_deliver_loops=3` 到顶后走 `gate deliver FAIL`（或 `scope --in` 膨胀）升 full 时，`promoteGateSetToFull` 同时把这条因预算而起的 `MAX_ITERATIONS` BLOCKED 翻回 `IN_PROGRESS` 并清掉过期的 `intervention_needed`（progress 行带 `status=BLOCKED→IN_PROGRESS`）；显式写 BLOCKED 的 gate 与 STAGNATION 不解除。此前升 full 后 run 仍显示 BLOCKED + 过期原因，与「继续走五 gate」不符。skill / 文档同步：预算到顶**不**自动升档，出口才升。合约：`tests/jj-ralph-contract.test.mjs`（P2+a 升档用例 d3–d5）。
