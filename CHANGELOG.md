...
- **人格标签 / 反 Start-broad / 活审查占位**：spawn `description` 必须以 `[implementer]` / `[reviewer]` / `[research]` 开头（禁止 `[reviewer] local changes`）；prompt 第一段禁止 Start broad / 全库 grep / list_dir；ASSIGNMENT 必须列精确文件（禁止「以现有代码为准」；禁止贴样本 session/commit id，工人会 `git show`）；same 入职拆成调研/实施两份；调研派单 listed 路径，不要「先搜再读」。活着的 `[reviewer]` 未结束时 `$jj-same` 不得再 spawn（占位胜过「本轮必须 spawn」）。G-ralph-4 / G-review-4 / G-same-2。样本 miss：`01a08fa6`。靶场：`.tmp/persona-flow-family/` TASK1–HANDOFF。合约：`tests/ralph/assets.contract.mjs`、`tests/jj-review-contract.test.mjs`、`tests/jj-same-contract.test.mjs`。
- **派遣前先告诉用户这一步**：主进程在 spawn 开发 / 审查 / 调研 / 交接实施子代理之前，先发一行用户可见进度（如「派遣前端开发实现任务」「派遣reviewer审查改动代码」），不要干等。Grok 可能把工具调用之后的正文压到子代理返回才显示，所以这行必须写在 spawn 之前。G-ralph-2 / G-review-2。样本 miss：`01a08fa0`。合约：`tests/ralph/assets.contract.mjs`、`tests/jj-review-contract.test.mjs`、`tests/jj-same-contract.test.mjs`。
- **对话记账按客服文档**：`$jj-ralph` / `$jj-same` / `$jj-dispatch` / `$jj-review` 对话路径不跑 `ralph_ops` / `jj ralph`（含 locate / init / deliver-attempt / gate / finalize）。记账写 `progress.md` + 双写 `run.json`；归档搬 `completed/` + `index.md`。G-ralph-3。机械 CLI 仍给 CLI 用户。合约：`tests/ralph/assets.contract.mjs`、`tests/jj-same-contract.test.mjs`、`tests/jj-dispatch-contract.test.mjs`。
- **审查按客服文档约束**：对话路径写 `ASSIGNMENT-REVIEW` + `findings.md` + `REV-n.json`（含 `run.json.review.reviews[]` 与 `accept_layers.judgment`），不跑 `review-record` / `context --review`。`HIGH`→`high`。先回 `[OK]`/`[BLOCK]` 再落盘。本轮文案/样式 skip。审查员只读 listed 文件，禁止整仓 grep。G-review-3（样本 `01a08ea0`）。命令入口与 strict 门禁错误指向 `$jj-review` 文档，不教 CLI。机械 CLI 仍保留。合约：`tests/jj-review-contract.test.mjs`、`tests/ralph/review.contract.mjs`、`tests/ralph/conversation.contract.mjs`、`tests/ralph/assets.contract.mjs`。
- **交接按客服派单 + 入职**：conversational `$jj-same` 先写本轮 `ASSIGNMENT-RESEARCH` / `ASSIGNMENT-HANDOFF`（目标仓调研后再实施），spawn 必须带入职 prefix；`$jj-dispatch` 的 `distribution_prompt` 只是统筹索引，不是工人说明书。G-same-1（样本 `01a08e5b` `d53a16510`）。不把 `onboarding.md` / `assignment.md` 挂在 same 入口。合约：`tests/jj-same-contract.test.mjs`、`tests/jj-dispatch-contract.test.mjs`。
- **Ralph 入口按客服 team-lead**：主对话只对齐、写派单、本轮 `spawn_subagent`（`jj-implementer`，缺失则 `general-purpose`），子代理做完再回报；禁止在 parent 改业务代码或把 skill `references/` 当开机清单。删除 `tiny-example.md`。G-ralph-2（样本 `01a08e5b`）。`$jj-review` 同样：parent 不审、不启动读 reference。命令入口也不点名那五份手册。不宣称端到端提速百分比。合约：`tests/ralph/assets.contract.mjs`、`tests/jj-review-contract.test.mjs`。
- **派单提示词 + 闭环对齐客服**：`ASSIGNMENT-TASK` / `ASSIGNMENT-REVIEW` / `ASSIGNMENT-FIX` 用客服正文（来自 team-lead、先确认再开工、读这些 / 交付 / 不要改、短句汇报等 Task n+1）。闭环：一刀一派 → 子代理汇报后停 → 验证通过写改动摘要 → 大功能审查 → `[OK]`/`[WARN]` 后用户验收 → accept。小改跳过审查。合约：`tests/ralph/assets.contract.mjs`。
- **审查改走客服规范**：`$jj-review` 写 `ASSIGNMENT-REVIEW` 并 spawn 只读 reviewer；**禁止**宿主 `/review` / `[reviewer] local changes`（含 unbound 与「当前的全部改动」——加宽派单文件列表仍 spawn）。检查维度：类型安全、null 处理、API 契约、回归。产出 `findings.md`：HIGH/MEDIUM/LOW + `[OK]`/`[WARN]`/`[BLOCK]`；双写 `REV-*.json`（`[OK]`/`[WARN]`→PASS，`[BLOCK]`→NEEDS_CHANGES）。大功能（non-tiny）deliver 后 `next=review`；`tiny` 小改跳过审查。G-review-1 follow-up delta 不变。G-review-2。样本：`EP-20260910` bound 首次 52–96 tools。合约：`tests/jj-review-contract.test.mjs`、`tests/ralph/conversation.contract.mjs`。
- **Ralph 派单文件 + 独立子代理**：当前切片写 `assignments/ASSIGNMENT-TASK<n>.md`（读这些 / 交这些 / 不要改那些 / 做完等下一刀），spawn 只吃该文件，不吃聊天。对话面不再教机械 `--lite` 封顶。G-ralph-1。
...
- **install-skill 升级补缺失 skill**：已有 `jj-ralph` 等文件时不再整组 `target-exists` 拒绝。默认补上新 skill（如 `jj-init`），不覆盖已有文件；`--force` 才刷新旧副本。现场：全平台装过旧版后对话里没有 `/jj-init`。合约：`tests/install-skill.test.mjs`。
- **skill-en-zh-rewrite 不进用户端**：仓库维护 skill，不进 npm 包、不随 `install-skill` 分发；重装时清掉宿主目录残留。合约：`tests/install-skill.test.mjs`。
