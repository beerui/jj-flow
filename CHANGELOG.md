...
- **Ralph 工作区 P2+ lite exec-plan 关闭**：`docs/exec-plans/completed/2026-09-03-ralph-plans-workspace-p2-lite.md`（P2+a + P2+b 勾完并附验收证据）；设计 [Ralph 任务工作区 `.plans` 化改造](docs/design-docs/ralph-plans-workspace.md) §3.8 / §4 P2+ 标落地，整体标 Implemented。`lab:check` 因 sibling gym pin（`8e51498`）在 `main` 同样失败，另行更新 pin。
