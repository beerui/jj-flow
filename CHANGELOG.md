...
- **Ralph 自动结案补录（§4.5 / §4.9.4–5）**：仓内补录 [ralph-auto-closeout](https://github.com/beerui/jj-flow/blob/main/docs/design-docs/ralph-auto-closeout.md)（标明非修订 10 原文；表格外 code span 用 `|`）。`locate` 行带 `next` / `warning` / `closeout`。`jj install-skill --platform agents`（`--platform all` 含此端）写入 `~/.agents/skills` + `commands`，安装时清 retired。`jj ralph remediate` 默认 dry-run，`--yes` 只处理 `finalize` 与 `migrate`。主路径仍见 PR #30。合约：`tests/jj-ralph-contract.test.mjs` **56/56**；全量 `node --test tests/*.test.mjs` **381/381**；`ralph:check` in_sync；`check` / `harness:check` / `harness:gc` / `scenario:check` / `host:trial` / `docs:check` / `evaluated:check` 通过；`git diff --check` 干净；`jj install-skill --platform all --force` 写入 `~/.agents`；本仓 `remediate` `count=0`。`lab:check` 为既有 `LAB-ROOT-MISSING`。
- **Ralph 自动结案（auto-closeout）**：accept PASS 后 MUST `finalize`。`status` 两处输出层（CLI `renderRalphStatusText` / `--json`，以及 skill SSOT `ralph_ops.mjs` `printJson`）带 `next`；`phase=ARCHIVE` 仍在活跃根（含 resume 后、rollback 前）打中性第二道告警「未完成收尾——先跑 gate/status 核对」。缺省 `gate_set` 显示 `undefined`（`effectiveGateSet` 行为仍按 full）。`writeRalphIndex` 内部 try/catch 降级，避免 `archiveRun` 主写入成功后假报失败。`jj ralph locate` / `ralph_ops locate` 覆盖扁平 `task-*` 与 `completed/`；jj-review Locate 同步。合约：`node --test tests/jj-ralph-contract.test.mjs` **55/55**；全量 `node --test tests/*.test.mjs` **379/379**；`ralph:check` in_sync（15 files）；`check` / `harness:check` / `harness:gc` / `scenario:check` / `host:trial` / `docs:check` / `evaluated:check` 通过；`git diff --check` 干净；`jj install-skill --platform all --force` 成功。本环境 `npm test`（`node --test tests`）在 Node 22 把目录当模块；`lab:check` 为既有 `LAB-ROOT-MISSING`（无 sibling gym）。合入：PR #30 / `d377acb`。
