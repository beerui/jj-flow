...
3. map-find **只跑 CLI**，不要整读 `business-map.json`。单点跟 tiny-example。只有 intensity（tiny/standard/strict），**不要** `--lite`。截图 / 「这里」先读图。需求确认不了先问，写 `## 存疑`，不要猜着做。用户说「先不写代码」只写 Goal + 存疑、不过关。同会话「继续 / 按审查改 / 改坏了」→ resume，禁止 init（CLI `--thread-id` / `host.thread_id` 也算同会话）。审查只出 findings，等用户说「按审查改」再改。
...
5. 同操作失败最多 2 次后换策略；校正写 `instruction-correction.md`，审查者只读、不写 `AGENTS.md`。
   tiny 默认不写空的 `## 存疑`；确认不了或先不写代码时写存疑并先问。
6. 收工顺序：review（可先 working_tree）→ commit → `review-record --review-scope commit` → `gate accept PASS` → **MUST** `finalize` → `$jj-end`。`working_tree` PASS 不能当落地。`index.md` 出现 `## 归档提示` 时只提示、不自动归档；拿不准先问。
