...
1. 解析脚本：repo skill → `$CODEX_HOME/skills/jj-ralph/scripts/` → `jj ralph`。已知同会话任务直接 `context --run-id`；未知编号用 index / 简明 `locate`，需要全部候选才 `--details`。
...
当前阶段优先使用 `context` 的短合同、验证尾部与范围预检；缺少具体规则时才读 `phases.md` 对应章节，不在启动时整读全部 references。审查用 `context --review` 一次交接，`--context-file` 校验后写回；范围变化刷新材料。机械目录 `ops.md`，恢复见 `post-complete-continue.md`、`rollback.md`。
