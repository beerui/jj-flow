...
- **`$jj-ralph` / `/jj-ralph`**：任务全流程闭环（分析 → 计划 → 验收 → 归档）与能力地图
...
它解决的是「任务如何可追溯地做完一件事」以及「多个已分叉项目如何协同改同一能力」。**产品定位是项目编排协议与工作流**。事实来源是 run ledger、control-plane、Git commit 与验证证据，不是聊天状态。
